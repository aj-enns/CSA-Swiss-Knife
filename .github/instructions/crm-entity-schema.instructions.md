...
## Team Structure (Three Layers)

MSX has three distinct team concepts at different scopes. Do not conflate them.

| Layer | Scope | Mechanism | Tool | What it represents |
|-------|-------|-----------|------|--------------------|
| **Deal Team** | Opportunity | `msp_dealteams` entity + Access Team | `manage_deal_team` or `crm_query` on `msp_dealteams` | Who's working the opportunity — populates the "Deal Team" tab in MSX. Has owner flag (`msp_isowner`). |
| **Milestone Team** | Milestone | D365 Access Team (template: "Milestone Team") | `manage_milestone_team` (list/add/remove) | Who's working a specific milestone — scoped per-milestone, independent of deal team. Populates the "Milestone Team" tab in MSX. |
| **Milestone Owner** | Milestone | `_ownerid_value` lookup on `msp_engagementmilestones` | `get_milestones` / `crm_query` | Single accountable person for the milestone. |

- Deal team members are NOT automatically on milestone teams — they are separate access teams.
- Milestone team members may differ from milestone owner — the owner is the single accountable person; the team is everyone contributing.
...
- `manage_milestone_team` is always available when milestone GUIDs are known.
